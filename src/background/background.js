...
function handleSetProxy(proxy, sendResponse) {
    ...
    if (proxy.id === 'direct' || proxy.ip === 'DIRECT') {
    ...
    } else if (proxy.id === 'system' || proxy.ip === 'SYSTEM') {
        // Use system proxy settings
        config = {
            mode: "system"
        };
    } else if (proxy.id === 'pac-demo' || proxy.ip === 'PAC_SCRIPT') {
        // PAC script demo - returns DIRECT for educational purposes
        ...
        // Regular proxy configuration (for future real proxies)
        if (!proxy.port) {
            sendResponse({ 
                success: false, 
...
function handleFetchProxies(sendResponse) {
    try {
        // Working proxy list with real options for testing
        var proxies = [
            {
                id: 'direct',
                ip: 'DIRECT',
            ...
            },
            {
                ...
                ip: 'SYSTEM',
                port: 0,
                country: '⚙️ Use System Proxy Settings',
                protocol: 'system',
                anonymity: 'system'
            },
            {
                ...
                ip: 'PAC_SCRIPT',
                port: 0,
                country: '📜 PAC Script Demo (Educational)',
                protocol: 'pac_script',
